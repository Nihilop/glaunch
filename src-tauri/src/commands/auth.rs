...
pub async fn auth_steam_callback(identity: String) -> Result<String, String> {
    ...
    let auth = SteamAuth::new(api_key);
    let steam_id = auth.extract_steam_id(&identity)
...
pub async fn auth_battlenet_callback(code: String) -> Result<String, String> {
    let client_id = std::env::var("BATTLENET_CLIENT_ID")
        .map_err(|_| "BATTLENET_CLIENT_ID not set")?;
    let client_secret = std::env::var("BATTLENET_CLIENT_SECRET")
        .map_err(|_| "BATTLENET_CLIENT_SECRET not set")?;
